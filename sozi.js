...
module(this, "sozi.animation", function (exports, window) {
	...
	if (sozi.document.frames[index].transitionTargetstyle) {
	   ...
	   for (var idt in objlist) {
              var target = document.getElementById(idt);
	      for (var attr in objlist[idt]) {
	        target.setAttribute(attr, objlist[idt][attr]);
              }
           }
        }
     }
...
//	if (attr == 'transition-duration-ms')  alert(attr+'='+value+'//'+DEFAULTS[attr]+':'+res);
	...
	if (objstrg) {
	  ...
	  objmod = {old:objold,
	            new:objnew};
	  var objlist = eval("({" + readAttribute(soziElement, attr) + "})");
	  for (var idt in objlist) {
	    objold[idt] = {};
	    objnew[idt] = {};
	    var target = document.getElementById(idt);
	    if (target) {
	      for (var attr in objlist[idt]) {
	        objold[idt][attr] = target.getAttribute(attr, target.getAttribute(attr));
	        if (attr == 'style') objnew[idt][attr] = objold[idt][attr]+';'+objlist[idt][attr];
	        else                 objnew[idt][attr] = objold[idt][attr]+' '+objlist[idt][attr];
              }
	    }
          }
        }
    ...
    function readLayerProperties(frame, idLayer, soziElement) {
        ...
        if (typeof layer.transitionZoomPercent === "undefined" || soziElement.hasAttributeNS(SOZI_NS, "transition-zoom-percent")) {
            layer.transitionZoomPercent = parseInt(readAttribute(soziElement, "transition-zoom-percent"), 100);
        }
        if (typeof layer.transitionProfile === "undefined" || soziElement.hasAttributeNS(SOZI_NS, "transition-profile")) {
            layer.transitionProfile = sozi.animation.profiles[readAttribute(soziElement, "transition-profile") || "accelerate-decelerate"];
        }
